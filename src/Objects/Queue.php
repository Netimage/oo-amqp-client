...
class Queue implements RabbitMqObjectInterface, QueueInterface
{
	 ...
	 * @param bool                $autoDeclare Automatically declare the queue
	 */
    public function __construct(Client $client, $name, array $consumers = [], $autoDeclare = true)
    {
        $this->client = $client;
    ...
    public function init(AMQPChannel $amqpChannel)
    {
        if (!$this->init) {
            ...
            if (substr($this->name, 0, 4) !== 'amq.' && $this->autoDeclare) {
